...
export function AboutHeroMobile() {
  ...
  return (
    <section
      id="about-hero"
      className="relative w-full bg-[#F5F2EC] overflow-hidden pt-24 pb-6 px-5 flex flex-col justify-between font-sans transition-colors duration-300"
    >
      ...
      <div className="w-full relative z-20">
        ...
        >
          <motion.span
            ...
            className="text-[#F97316] font-semibold tracking-[0.2em] text-[10px] uppercase block"
          >
            {HERO_DATA.label}
          </motion.span>
          ...
          <motion.h1
            variants={textRevealVariants}
            className="font-serif text-3xl font-bold text-[#163B65] tracking-tight leading-[1.18]"
          >
            {HERO_DATA.headlineFirstLine} <br />
            {HERO_DATA.headlineSecondLine} <br />
            <span className="italic font-normal text-[#163B65]/90">
      ...
      >
        ...
        <div className="p-5 flex flex-col justify-between h-full">
          <div className="flex justify-between items-start gap-4">
            <p
              className={`font-serif text-sm font-semibold tracking-wide ${isExpanded ? "text-[#F5F2EC]" : "text-[#163B65]"}`}
            >
              {isExpanded ? "Closing Vision Lookbook" : "Open Spatial Curation"}
            </p>
            <span
              className={`text-xs font-mono font-bold shrink-0 ${isExpanded ? "text-[#F97316]" : "text-[#163B65]/40"}`}
            >
              {isExpanded ? "—" : "+"}
            </span>
          </div>
          ...
          <AnimatePresence initial={false}>
            {isExpanded && (
              ...
              >
                ...
                <div className="relative aspect-[16/10] w-full mt-4 overflow-hidden bg-[#163B65]/5 border border-white/10">
                  <motion.div
                    initial={{ scale: 0.96 }}
                    animate={{ scale: 1 }}
                    exit={{ scale: 0.96 }}
                ...
                <motion.div
                  ...
                  className="mt-4 pb-1 text-xs leading-relaxed text-[#F5F2EC]/90 max-w-sm"
                >
      ...
      <div className="w-full border-t border-[#163B65]/10 pt-4 mt-6 flex justify-between items-center z-20">
        <span className="font-sans text-[9px] uppercase tracking-widest text-[#475569]/60 font-medium">
        ...
        <button
          ...
          type="button"
          className="inline-flex items-center gap-x-1.5 font-sans text-xs font-bold uppercase tracking-widest text-[#163B65] active:text-[#F97316] transition-colors"
        >
