...
export function JourneyTimelineMobile() {
  ...
  return (
    <section id="the-shift" className="py-16 px-5 bg-background-main">
      {/* Structural Editorial Header Block */}
      <div className="w-full max-w-md mx-auto mb-8 font-sans">
        <span className="text-accent font-semibold tracking-[0.2em] text-[10px] uppercase block mb-2">
      ...
      <motion.div
        ...
        className={`w-full max-w-md mx-auto border cursor-pointer select-none overflow-hidden relative group transition-colors duration-300 ${
          isExpanded
            ...
            : "bg-background-secondary/40 border-border-main text-text-primary"
        }`}
      >
        ...
        <div className="p-4 flex flex-col justify-center min-h-[64px]">
          ...
          <div className="w-full flex justify-between items-center gap-4">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <motion.div
                ...
                className={`w-7 h-7 rounded-full border flex items-center justify-center font-mono text-[9px] font-bold shrink-0 transition-colors duration-300 ${
                  isExpanded
                    ...
                    : "border-border-main bg-background-main text-text-primary"
                }`}
              >
                t°
              </motion.div>
              ...
              <p
                className={`font-serif text-base font-bold tracking-tight truncate ${isExpanded ? "text-white" : "text-text-primary"}`}
              >
                {isExpanded
            ...
            <span
              className={`text-xs font-mono font-bold shrink-0 pr-1 ${isExpanded ? "text-accent" : "text-text-primary/40"}`}
            >
              {isExpanded ? "—" : "+"}
            </span>
          </div>
            ...
            {isExpanded && (
              ...
              >
                ...
                <motion.div
                  ...
                  className={`w-full h-[1px] mt-4 origin-left ${isExpanded ? "bg-white/20" : "bg-border-main"}`}
                />

                {/* Immersive Micro-Scale Graphic Box */}
                <div className="relative w-full mt-4">
                  <div className="relative aspect-[16/10] w-full overflow-hidden bg-text-primary/5 border border-white/10">
                    <motion.div
                      initial={{ scale: 0.96 }}
                      animate={{ scale: 1 }}
                      exit={{ scale: 0.96 }}
                    ...
                    >
                      ...
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-navbar-active/50 to-transparent pointer-events-none" />
                    </motion.div>
                  </div>
                </div>
                ...
                <div className="relative max-h-[220px] overflow-y-auto mt-4 pr-1 text-xs sm:text-sm leading-relaxed font-sans space-y-4 text-white/90 scrollbar-none">
                  ...
                  <p className="text-accent font-medium font-serif italic text-base tracking-tight border-l border-accent pl-3 py-0.5 my-2">
