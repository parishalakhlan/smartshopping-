...
export function CultureSectionMobile() {
  ...
  return (
    <section id="our-beliefs" className="py-16 px-5 bg-background-main">
        ...
        {BELIEF_PILLARS.map((pillar) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`absolute top-0 left-0 right-0 h-[2px] bg-accent transition-transform duration-400 origin-left ${
              ...
              <div className="p-4 flex flex-col justify-center min-h-[64px]">
                ...
                <div className="flex justify-between items-center gap-4 select-none">
                  <div className="flex items-baseline gap-x-3">
                    <span className="font-serif text-[11px] font-bold text-accent tracking-widest">
                      {pillar.num}
                    </span>
                    <h3
                      className={`font-serif text-lg font-bold tracking-tight ${isOpen ? "text-white" : "text-text-primary"}`}
                    >
                      {pillar.title}
                    </h3>
                  </div>
                  ...
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${
                      isOpen
                        ...
                        : "bg-text-primary/5 text-text-primary group-hover:bg-text-primary group-hover:text-white"
                    }`}
                  >
                    {isOpen ? (
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="pt-4 text-xs sm:text-sm leading-relaxed space-y-3 relative max-h-[180px] overflow-y-auto pr-1 scrollbar-none">
                        <p
                          className={`font-medium ${isOpen ? "text-background-secondary" : "text-text-primary"}`}
                        >
                          {pillar.excerpt}
                        </p>
                        <p
                          className={`${isOpen ? "text-background-secondary/80" : "text-text-secondary/90"}`}
                        >
                          {pillar.extended}
                        </p>
