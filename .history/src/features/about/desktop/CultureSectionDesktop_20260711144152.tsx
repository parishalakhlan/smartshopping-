  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-8">
        ...
        <div className="w-full border-t border-border-main font-sans">
          {BELIEF_PILLARS.map((pillar) => {
            const isOpen = activePillar === pillar.id;
            return (
              <div
                key={pillar.id}
                className="border-b border-border-main py-8 transition-colors duration-300"
              >
                <div
                ...
                >
                  <div className="flex items-center gap-x-12">
                    <span className="font-serif text-sm font-bold text-accent tracking-widest block py-1">
                      {pillar.num}
                    </span>
                    <h3 className="font-serif text-2xl sm:text-3xl text-text-primary font-bold group-hover:text-accent transition-colors duration-300">
                      {pillar.title}
                    </h3>
                  </div>

                  <div className="w-10 h-10 rounded-full bg-background-secondary flex items-center justify-center text-text-primary group-hover:bg-text-primary group-hover:text-white transition-colors duration-300 shrink-0">
                    {isOpen ? (
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="pt-4 pb-2 text-base sm:text-lg leading-relaxed font-normal space-y-3">
                        ...
                        <p className="text-sm sm:text-base text-text-secondary/90">
                          {pillar.extended}
                        </p>
                      </div>
                    </motion.div>
