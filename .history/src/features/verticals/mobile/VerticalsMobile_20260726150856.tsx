...
export function VerticalsHeroMobile() {
  ...
  return (
    <section className="relative min-h-[75vh] w-full max-w-full box-border flex flex-col justify-end px-5 sm:px-8 pb-10 pt-10 bg-background-main overflow-hidden font-sans">
      ...
      <div className="absolute inset-0 z-0">
        <Image
          ...
          fill
          priority
          className={`object-cover ${hero.image.objectFit || "object-center"}`}
          sizes="100vw"
        />

        {/* High-Contrast Scrim: Dark gradient guarantees crisp text legibility over light images */}
      ...
      <div className="relative z-10 space-y-4 sm:space-y-6 max-w-md w-full box-border">
        ...
        <span className="text-[10px] sm:text-[11px] uppercase tracking-[0.3em] text-accent font-extrabold block drop-shadow-xs">
          {hero.tagline}
        </span>

        {/* High-End Editorial Typography Statement */}
        <h1 className="text-3xl sm:text-4xl font-light tracking-tight text-white leading-[1.18] font-luxury-heading drop-shadow-sm">
        ...
        {/* Body Copy */}
        <p className="text-xs sm:text-sm text-white/85 font-normal leading-relaxed max-w-xs drop-shadow-xs">
          {hero.description}
        </p>
      </div>
    </section>
      ...
      <div className="relative w-full box-border">
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            ...
            {/* Metrics & Impact */}
            <div className="pt-4 border-t-2 border-border-main/40 grid grid-cols-2 gap-3 bg-surface p-3 border border-border-main">
              <div className="space-y-0.5">
                <span className="text-[9px] text-text-secondary uppercase tracking-[0.15em] block font-extrabold">
                  Scale Reach
                </span>
                <span className="text-xs sm:text-sm text-accent font-bold block truncate">
                  {activeData.impact}
                </span>
              </div>

              <div className="space-y-0.5 border-l border-border-main/50 pl-3">
                <span className="text-[9px] text-text-secondary uppercase tracking-[0.15em] block font-extrabold">
                  Horizon Target
                </span>
                <span className="text-xs sm:text-sm text-text-primary font-bold block truncate">
                  {activeData.vision}
                </span>
              </div>
            </div>
            ...
            <button
              onClick={handleNext}
              className="w-full py-2.5 bg-surface hover:bg-background-main border border-border-main text-[10px] uppercase tracking-[0.2em] font-extrabold text-text-primary flex items-center justify-center gap-1.5 transition-colors active:scale-[0.99]"
            >
              <span>Explore Next Cluster</span>
