...
interface GrowthMapRevealProps {
  config?: typeof partnersConfig;
}

export default function GrowthMapReveal({
  config = partnersConfig,
}: GrowthMapRevealProps) {
  const [activeZone, setActiveZone] = useState<string | null>(
    config.regionalRegions[0].id,
  );
  const currentZone = config.regionalRegions.find((r) => r.id === activeZone);
