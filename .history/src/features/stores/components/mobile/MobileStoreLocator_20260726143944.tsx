...
export function MobileStoreLocator({ data }: MobileStoreLocatorProps) {
  ...
  return (
    <div className="w-full bg-background-main min-h-screen text-text-primary overflow-hidden md:hidden font-sans">
      ...
      <header className="sticky top-0 z-30 bg-background-main/95 backdrop-blur-md border-b border-border-main/50 px-4 pt-6 pb-3 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="p-1.5 rounded-lg bg-accent/10 text-accent">
              <StoreIcon className="w-4 h-4" />
            </span>
            <h1 className="font-serif text-xl font-bold text-text-primary tracking-tight">
              Showroom Locator
            </h1>
          </div>
          <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-background-secondary text-text-secondary border border-border-main/40">
        ...
        <div className="relative w-full">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-text-secondary/60" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => {
            ...
            className="w-full pl-9 pr-8 py-2.5 text-xs bg-background-secondary/60 border border-border-main/60 rounded-xl focus:outline-none focus:border-accent transition-colors shadow-xs"
          />
          {searchQuery && (
            <button
        ...
        {!searchQuery && (
          <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none -mx-4 px-4 snap-x">
            {availableStates.map((state) => {
              ...
              return (
                <button
                  ...
                  className={`snap-start shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-semibold tracking-wider uppercase transition-all duration-200 border ${
                    isExpanded
      ...
      <main className="px-4 py-4 space-y-3 pb-24">
        {availableStates.length === 0 ? (
          ...
          availableStates.map((stateName) => {
            ...
            // When user is searching, auto-expand states; otherwise allow accordion behavior
            ...
            return (
              <div
                ...
                className="scroll-mt-36 rounded-2xl border border-border-main/60 bg-background-main overflow-hidden shadow-xs"
              >
                {/* STATE GROUP HEADER */}
                <button
                  onClick={() =>
                  ...
                  className="w-full px-4 py-3.5 bg-background-secondary/40 flex items-center justify-between border-b border-border-main/30 select-none text-left"
                >
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-accent" />
                    <h2 className="font-serif text-sm font-bold text-text-primary tracking-wide">
                      {stateName}
                    </h2>
                    <span className="text-[10px] font-bold text-text-secondary/70 bg-background-main px-2 py-0.5 rounded-full border border-border-main/40">
                      {stateStores.length}{" "}
                  ...
                  <div className="flex items-center gap-1.5 text-text-secondary text-[11px]">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-accent">
                      {isStateExpanded ? "Collapse" : "View All"}
                    </span>
                    <ChevronDown
                    ...
                    >
                      {stateStores.map((store) => {
                        ...
                        return (
                          <div
                            key={store.id}
                            className="bg-background-main transition-colors"
                          >
                            ...
                            <AnimatePresence>
                              {isStoreExpanded && (
                                <motion.div
                                  ...
                                  animate={{ opacity: 1, height: "auto" }}
                                  exit={{ opacity: 0, height: 0 }}
                                  className="px-3.5 pb-3.5 pt-1 bg-background-secondary/30 space-y-3"
                                >
                                  {store.image?.src && (
                                    <div className="relative w-full h-36 rounded-xl overflow-hidden bg-background-secondary">
                                      <Image
                                        src={store.image.src}
