...
export function MobileStoreExperience() {
  return (
    <section className="bg-background-main py-16 px-6 font-sans border-t border-border-main md:hidden overflow-hidden">
      <div className="space-y-8">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 pt-2 scrollbar-none snap-x snap-mandatory -mx-6 px-6">
          {GAL_IMAGES.map((img, i) => (
            <div
              key={i}
              className="w-[260px] h-80 rounded-none overflow-hidden relative border border-border-main bg-background-secondary shrink-0 snap-start shadow-sm"
            >
              <Image
                src={img.url}
                alt={img.label}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-hero-overlay via-button-primary-bg/30 to-transparent opacity-95" />

              {/* Text Meta Layer */}
              <div className="absolute bottom-5 left-5 right-5 text-left z-10">
                <span className="text-[9px] font-mono font-bold text-accent tracking-[0.2em] uppercase block mb-1">
                  ZONE LAYERING 0{i + 1}
                </span>
                <p className="font-serif font-bold text-sm text-button-primary-text tracking-tight leading-snug">
                  {img.label}
                </p>
              </div>
