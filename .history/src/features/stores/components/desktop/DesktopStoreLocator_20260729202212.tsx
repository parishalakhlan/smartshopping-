...
export function DesktopStoreLocator({ data }: DesktopStoreLocatorProps) {
  ...
  useEffect(() => {
    ...
    console.log(
      "State values:",
      stores.map((s) => s.state),
    );
    console.log("Unique states:", [...new Set(stores.map((s) => s.state))]);
    console.log(
      "State counts:",
      stores.reduce(
        (acc, store) => {
    ...
    console.log("Regional footprints from props:", regionalFootprints);

    // Log duplicate state names
    ...
    const duplicates = Object.entries(stateCounts)
      .filter(([_, count]) => count > 1)
      .map(([state, count]) => `${state}: ${count} stores`);

    if (duplicates.length > 0) {
      console.log("⚠️ Duplicate states found:", duplicates);
    }
  }, [stores, regionalFootprints]);
