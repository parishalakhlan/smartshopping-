...
export function DesktopStoreExperience() {
  return (
    <section className="hidden md:block bg-background-main py-24 sm:py-32 px-4 sm:px-6 lg:px-8 font-sans border-t border-border-main">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {GAL_IMAGES.map((img, i) => (
            <div
              key={i}
              className="h-80 rounded-none overflow-hidden relative border border-border-main group bg-background-secondary"
            >
              <Image
                src={img.url}
                alt={img.label}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-hero-overlay via-button-primary-bg/20 to-transparent opacity-90 transition-opacity duration-500 group-hover:opacity-75" />

              {/* Floating Typography Label */}
              <div className="absolute bottom-5 left-5 right-5 text-left z-10">
                <span className="text-[9px] font-bold text-accent tracking-[0.2em] uppercase block mb-1">
                  ZONE LAYERING 0{i + 1}
                </span>
                <p className="font-serif font-bold text-base text-text-primary tracking-tight leading-snug">
                  {img.label}
                </p>
              </div>
