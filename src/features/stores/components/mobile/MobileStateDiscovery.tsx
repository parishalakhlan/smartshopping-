// src/features/stores/components/MobileStateDiscovery.tsx
"use client";
...
export function MobileStateDiscovery() {
  return (
    <section
      ...
      className="bg-[#F5F2EC] py-16 px-6 border-t border-[#163B65]/10 font-sans md:hidden overflow-hidden"
    >
      <div className="space-y-8">
        ...
        <div className="flex gap-4 overflow-x-auto pb-6 pt-2 scrollbar-none snap-x snap-mandatory -mx-6 px-6">
          {REGIONAL_FOOTPRINTS.map((footprint) => (
            <div
              key={footprint.id}
              className="bg-[#FFFFFF] p-6 rounded-none border border-[#163B65]/10 flex flex-col justify-between w-[280px] shrink-0 snap-start relative shadow-sm"
            >
              <div className="space-y-5 text-left">
                {/* Meta Numeric Strip */}
                <div className="text-[9px] font-mono font-bold tracking-widest text-[#F97316]">
                  ZONE // 0{footprint.id || footprint.name.length}
                </div>

                <div className="space-y-1">
                  <h3 className="font-serif text-xl font-bold text-[#163B65] tracking-tight">
                    {footprint.name}
                  </h3>
                  <p className="text-[11px] text-[#475569]/90 font-semibold tracking-tight">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {footprint.cities.slice(0, 3).map((city, idx) => (
                    <span
                      key={idx}
                      className="bg-[#F5F2EC]/70 text-[#163B65] text-[9px] font-bold px-2 py-0.5 border border-[#163B65]/5 uppercase tracking-wide"
                    >
                      {city}
                    </span>
                  ))}
              ...
              <a
                ...
                className="mt-8 pt-4 border-t border-[#163B65]/5 flex items-center justify-between text-[11px] font-bold text-[#163B65] uppercase tracking-wider"
              >
              ...
              <div className="absolute top-0 left-0 h-[2px] bg-[#F97316] w-full" />
            </div>
          ))}
        </div>
      </div>
