// src/features/stores/components/DesktopStoreLocator.tsx
"use client";
...
import { NORTH_INDIA_STORES } from "@/features/stores/data/stores";
import { REGIONAL_FOOTPRINTS } from "@/features/stores/data/stores-data";
...
export function DesktopStoreLocator() {
  ...
  return (
    <section
      ...
      className="hidden md:block w-full bg-[#FFFFFF] py-16 lg:py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans"
    >
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-stretch min-h-[720px]">
        {/* MAP PANEL (60%) */}
        <div className="w-full lg:w-[60%] flex flex-col justify-between relative bg-[#F5F2EC] overflow-hidden border border-[#163B65]/10 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 z-20 w-full bg-[#FFFFFF]/95 backdrop-blur-md p-5 border border-[#163B65]/5">
          ...
          <div className="relative flex-1 w-full min-h-[500px] flex items-center justify-center p-4">
            <div className="absolute bottom-6 left-6 z-20 flex flex-col bg-[#FFFFFF] border border-[#163B65]/10 p-1">
              <button
                onClick={() => setZoomLevel((p) => Math.min(p + 0.2, 1.6))}
                className="p-2 hover:bg-[#F5F2EC] text-[#163B65]"
                aria-label="Zoom In"
              >
                    ...
                    return (
                      <path
                        ...
                        onClick={() => {
                          ...
                          const match = NORTH_INDIA_STORES.find(
                            (s) =>
                              s.state.toLowerCase() ===
                              footprint.name.toLowerCase(),
                          );
                    ...
                    >
                      <div
                        className={`w-7 h-7 rounded-full flex items-center justify-center font-semibold text-xs transition-all duration-300 ${
                          isSelected
                            ? "bg-[#F97316] text-[#FFFFFF] ring-4 ring-[#F97316]/20"
                      ...
                      <span
                        className={`text-[9px] font-bold uppercase tracking-wider mt-1.5 bg-[#FFFFFF]/95 px-1.5 py-0.5 border ${
        ...
        <div className="w-full lg:w-[40%] flex flex-col space-y-6">
          ...
          <div className="flex-1 overflow-y-auto space-y-4 max-h-[600px] pr-2 border-t border-[#163B65]/5 pt-2">
              ...
              {filteredStores.map((store) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`bg-[#FFFFFF] rounded-none border p-5 transition-all duration-400 cursor-pointer relative overflow-hidden text-left ${
                      isCurrent
                        ? "border-[#163B65] bg-[#F5F2EC]/20"
                        : "border-[#163B65]/10 hover:border-[#F97316]/40"
                    }`}
                  >
                    <div className="flex justify-between items-center pb-3 border-b border-[#163B65]/5">
                      <div className="text-[10px] font-bold text-[#475569]/70 tracking-widest uppercase">
                        {store.brands.join("  //  ")}
                      </div>
                      <span className="text-[#10B981] text-[9px] font-bold uppercase tracking-widest flex items-center gap-1">
                    ...
                    <div className="mt-4 grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
                      <div className="sm:col-span-8 space-y-1.5">
                        <h4 className="font-serif text-lg font-bold text-[#163B65] tracking-tight">
                          {store.name}
                        </h4>
                      ...
                      <div className="sm:col-span-4 h-16 relative border border-[#163B65]/10 bg-[#F5F2EC] hidden sm:block">
                        <Image
                          src={store.image}
                          alt={store.name}
                          fill
                    ...
                    <div className="mt-5 pt-3 border-t border-[#163B65]/5 flex items-center justify-between">
                      <a
                        href={`tel:${store.phone}`}
                        className="text-xs font-bold text-[#475569] hover:text-[#F97316] uppercase tracking-wider inline-flex items-center gap-1.5"
                      >
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs font-bold text-[#FFFFFF] bg-[#163B65] hover:bg-[#F97316] px-3 py-2 rounded-none uppercase tracking-wider inline-flex items-center gap-1.5"
                      >
