...
export default function FashionExperienceMobile({
  ...
  return (
    <section className="relative h-[100vh] w-full bg-[#F5F2EC] flex flex-col justify-between overflow-hidden px-5 py-12 md:hidden select-none">
      {/* Dynamic Native Application Mini Header Layout */}
      <div className="w-full pt-4">
        <span className="text-[#F97316] font-sans font-medium tracking-[0.2em] text-[10px] uppercase block mb-2">
          {data.tagline}
        </span>
        <h2 className="font-serif text-3xl text-[#163B65] font-bold tracking-tight leading-[1.15] max-w-xs">
          {data.headerTitle}
        </h2>
      </div>
      ...
      <div className="w-full pb-2 flex justify-between items-center">
        <p className="font-sans text-[#475569] text-xs font-light tracking-wide">
...
function SwipeCard({
  ...
  return (
    <motion.div
      ...
      className="absolute inset-0 w-full h-full bg-white border border-[#163B65]/5 shadow-xl flex flex-col justify-between p-4 cursor-grab active:cursor-grabbing origin-bottom"
    >
      {/* Immersive Photo Frame Fill */}
      <div className="relative w-full h-[70%] overflow-hidden bg-[#F5F2EC]">
        <Image
          src={moment.image}
          alt={moment.title}
          fill
      ...
      <div className="pt-4 pb-2 flex-1 flex flex-col justify-center">
        <h3 className="font-serif text-xl text-[#163B65] font-bold leading-tight mb-1">
          {moment.title}
        </h3>
        <p className="font-sans text-[#475569] text-xs leading-relaxed line-clamp-2">
          {moment.subtitle}
        </p>
      </div>
    </motion.div>
